...
function getMeasurePlace(meter, measureCount, isRecursive){
    ...
    if(measureCount === 0)
        return isRecursive ? MEASURE_PLACE.DOWNBEAT : MEASURE_PLACE.BEGINNING;
...
function solverRequestReconstruct(solverRequestDto){
    ...
    return new SolverRequestDto(
        exerciseReconstruct(solverRequestDto.exercise),
...
function FalseRelationRule(details, evaluationRatio){
    ...
    this.causedBySopranoOrBassSettings = function(prevChord, currentChord, prevVoice, currentVoice){
        ...
        if(isDefined(prevChord.bassNote.chordComponent) && prevChord.countBaseComponents("3") === 2 && prevChord.notes[prevVoice].baseChordComponentEquals("3"))
            return true;
